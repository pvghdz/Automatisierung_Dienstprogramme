...
"""
Code used to monitor and control devices connected to a Shelly switch.
It evaluates the power, current, and voltage of the shelly device and takes action according
to specified metrics.

This code was originally written to monitor water pumps (Förderpumpe) to keep them from 
...
logger = logging.getLogger("tl6.pump") # Get (or create) a named logger for your app/module.
logger.setLevel(logging.INFO) # Set the minimum severity level this logger will handle (INFO and above).

# Clean out any old handlers that might still point elsewhere
...
if __name__ == "__main__":
    ...
    device_timeout_thresholds = {"shellysw-extpump": 135, "shellysw-intpump": 375}
    
    monitors = []
    threads = []
    
    for dev_id, timeout in device_timeout_thresholds.items():
        m = ShellyMonitor(
            broker="<IP>",
            device_id=dev_id,
            client_id=f"<Name>-{dev_id}",             # Must have an uique client per device
            poll_interval=3,
            on_timeout_sec=timeout,                    # per-device timeout
        ...
        m.connect()                   # connect to broker and start background loop
        monitors.append(m)
        t = Thread(target = m.run, daemon = True)
        t.start()
        threads.append(t)
        
    # keep the main thread alive; clean shutdown on Ctrl+C
    try:
        while True:
            time.sleep(3600)
